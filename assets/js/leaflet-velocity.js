// Costa Viva -- copia local de leaflet-velocity@2.1.4 (onaci fork), con un
// parche propio: el grosor de cada trazo de partícula ahora escala con la
// misma cubeta de velocidad que ya fija el color (antes era un valor fijo,
// PARTICLE_LINE_WIDTH). Pedido explícito del usuario 2026-09-19: que el
// viento más fuerte se vea también más grueso, no solo más largo y más
// magenta. La librería original no soporta esto por opción, así que se
// vendoriza en vez de parchear en caliente desde el CDN. Ver el commit que
// introdujo este fichero para el diff exacto frente al original.
"use strict";
...
var Windy = function Windy(params) {
  ...
  var setOptions = function setOptions(options) {
    ...
    if (options.hasOwnProperty("lineWidth")) {
      PARTICLE_LINE_WIDTH_MAX = options.lineWidth;
      PARTICLE_LINE_WIDTH_MIN = PARTICLE_LINE_WIDTH_MAX * 0.35;
    }
    ...
    function draw() {
      ...
      buckets.forEach(function (bucket, i) {
        if (bucket.length > 0) {
          ...
          // Grosor unido a la misma cubeta de velocidad que ya fija el
          // color -- viento mas fuerte se ve mas grueso ademas de mas
          // largo y mas magenta (Costaviva, pedido explicito 2026-09-19).
          g.lineWidth = buckets.length > 1 ? PARTICLE_LINE_WIDTH_MIN + (PARTICLE_LINE_WIDTH_MAX - PARTICLE_LINE_WIDTH_MIN) * (i / (buckets.length - 1)) : PARTICLE_LINE_WIDTH_MAX;
